...
class Tree
{
	...
	public static  $arr = [];//路由树
	static public $end = "\n";
	static public $wildcard = '*';
	static $treeHandle = null;
	...
	{ //加锁怎么加？
		if (self::$treeHandle == null) { //加锁
			self::$treeHandle = new Tree();
		}
		$treeCache =  CacheDriver::register();
		$routeTree =  $treeCache->get('RouteTree');
		self::$arr = $routeTree==null?[]:$routeTree;//从缓存中拿一下路由
		return self::$treeHandle;
	}
	/**
	 * 插入一个节点
	 * 插入时允许使用通配符* 
	 ...
	 * @return boolean
	 */
	public function insert($path, $value,$expire = 60, $replace_exits = false, &$replaced = null)
	{
		assert(count($path));
		$left = $path;
		$cur = array_shift($left); //取出第一个元素
		if (!array_key_exists($cur, self::$arr)) { //检查self::$arr中有没有第一个元素
			self::$arr[$cur] = array();
		}
		$insert = false;
		...
		$treeCache =  CacheDriver::register();
		$treeCache->set('RouteTree', self::$arr,$expire);//会把insert的路由更新到缓存中
		return $insert;
	}
	/**
	 * 删除一个路径下的所有节点
	...
	public function erase(array $path)
	{
		...
		$key = $path[$size - 1];
		if ($size == 1) {
			if (array_key_exists($key, self::$arr)) {
				unset(self::$arr[$key]);
				return true;
			}
			return false;
		...
		$treeCache =  CacheDriver::register();
		$treeCache->set('RouteTree', self::$arr,$expire);//会把路由更新到缓存中
		return $res;
	}
	/**
	 * 遍历路径 
	 ...
	 * @return void
	 */
	public static  function treeToArray($tree, &$res)
	{
